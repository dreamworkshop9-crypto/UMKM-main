...
<body class="font-sans text-white antialiased" style="background:#0f172a">
    ...
    <div id="flash-msg" class="fixed top-24 right-4 z-[80] flex items-center gap-3 px-5 py-4 bg-slate-800 border border-brand-500/30 rounded-xl shadow-2xl max-w-sm toast-enter">
        ...
        <p class="text-sm text-slate-200">{{ session('success') }}</p>
    </div>
    @endif
    ...
    <nav id="navbar" class="glass-nav fixed top-0 left-0 right-0 z-50 border-b border-slate-700/30 transition-all duration-300">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button onclick="openWishlist()" class="relative w-10 h-10 flex items-center justify-center rounded-lg text-slate-400 hover:text-white hover:bg-slate-700/50 transition-all" title="Wishlist">
                        ...
                        @auth
                            ...
                            @endphp
                            <span id="wishlist-badge" class="{{ $wishlistCount > 0 ? '' : 'hidden' }} absolute -top-1 -right-1 w-5 h-5 bg-rose-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                                {{ $wishlistCount }}
                            </span>
                        @else
                    ...
                    <div class="hidden sm:block relative" id="dd-wrap">
                        <button onclick="document.getElementById('dd-menu').classList.toggle('hidden')" class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-slate-700/50 transition-all">
                            ...
                            <span id="nav-user-name" class="text-sm text-slate-300 max-w-[100px] truncate">{{ auth()->user()->name }}</span>
                        ...
                        <div id="dd-menu" class="hidden absolute right-0 top-full mt-2 w-56 bg-slate-800 border border-slate-700/30 rounded-xl shadow-2xl overflow-hidden z-50">
                            <div class="px-4 py-3 border-b border-slate-700/30">
                                <p id="dd-user-name" class="text-sm font-semibold text-white">{{ auth()->user()->name }}</p>
        ...
        <div id="mobile-menu" class="hidden md:hidden border-t border-slate-700/30">
            <div class="px-4 py-4 space-y-1">
                ...
                <form method="POST" action="{{ route('logout') }}" class="block" onsubmit="return confirm('Apakah Anda yakin ingin keluar?')">
                    @csrf
            ...
            <div class="grid lg:grid-cols-2 gap-12 items-center">
                ...
                <div class="relative fade-up hidden lg:block">
                    <div class="relative z-10 float-anim">
                        <img src="{{ asset('images/default-product.png') }}" alt="Sepatu Premium" class="w-full max-w-lg mx-auto rounded-3xl shadow-2xl shadow-black/30">
                    </div>
    ...
    <section class="py-16 border-t border-slate-700/20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex gap-3 overflow-x-auto scrollbar-hide pb-2" id="category-filter">
                ...
                <button onclick="setCategory('{{ $kat->id }}')" data-cat="{{ $kat->id }}" class="cat-btn flex-shrink-0 px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 bg-slate-800 text-slate-400 border border-slate-700/30 hover:border-brand-500/50 hover:text-white">
                    <i class="fa-solid fa-tag mr-2 text-xs"></i>{{ $kat->name }}
                </button>
                @endforeach
            </div>
        </div>
    ...
    <section id="produk" class="py-16 lg:py-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div id="product-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach($produk ?? [] as $p)
                    @include('front.partials.product_card', ['p' => $p])
                @endforeach
            </div>

            @if(isset($produk) && count($produk) >= 12)
    ...
    <button onclick="document.getElementById('search-input').value=''; setCategory('{{ $kategori->first()->id ?? "" }}');" class="mt-4 text-brand-400 hover:text-brand-300 text-sm font-medium">Reset Filter</button> 
</div>
    </section>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid lg:grid-cols-2 gap-16 items-center">
                <div class="relative fade-up">
                    <img src="{{ asset('images/default-product.png') }}" alt="Workshop" class="w-full rounded-2xl shadow-xl">
    ...
    <footer class="border-t border-slate-700/20 pt-16 pb-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
                ...
                <div><h4 class="text-sm font-semibold text-white uppercase tracking-wider mb-4">Kategori</h4><ul class="space-y-3">@foreach($kategori ?? [] as $kat)<li><a href="#" onclick="setCategory('{{ $kat->slug }}');document.getElementById('produk').scrollIntoView({behavior:'smooth'});return false;" class="text-sm text-slate-500 hover:text-brand-400">{{ $kat->name }}</a></li>@endforeach</ul></div>
            ...
            <div class="border-t border-slate-700/30 pt-8 text-center"><p class="text-xs text-slate-600">&copy; {{ date('Y') }} SALZA. All rights reserved.</p></div>
        </div>
    </footer>
    ...
    {{-- ===== ACCOUNT SIDEBAR (DASHBOARD PELANGGAN) ===== --}}
    @auth
            ...
            <div id="tab-acc-content-dashboard" class="space-y-6">
                ...
                <div class="p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl flex items-start gap-3 text-amber-400 text-xs leading-relaxed">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-slate-300">Anda memiliki {{ $unpaidOrders->count() }} pesanan yang belum dibayar. Silakan lakukan pembayaran dan unggah bukti pembayaran agar pesanan Anda dapat segera diproses.</p>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-slate-800/40 border border-slate-700/20 rounded-2xl p-5 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-bold text-white mt-0.5">{{ count($orders ?? []) }}</p>
                        </div>
                    </div>
                    @php
                    ...
                    <div class="bg-slate-800/40 border border-slate-700/20 rounded-2xl p-5 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-bold text-white mt-0.5">{{ $totalActive }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    @if(isset($orders) && count($orders) > 0)
                        @foreach(collect($orders)->take(3) as $order)
                        <div class="bg-slate-800/30 border border-slate-800 rounded-xl p-4 flex justify-between items-center text-sm">
                            <div>
                                <p class="text-xs text-brand-400 font-semibold uppercase tracking-wider">{{ $order->invoice }}</p>
                                <p class="text-xs text-slate-500 mt-0.5">{{ $order->created_at->format('d M Y, H:i') }}</p>
                                <p class="text-sm font-bold text-white mt-1">Rp {{ number_format($order->total, 0, ',', '.') }}</p>
                            </div>
                            <div class="text-right">
                                ...
                                @else
                                    <span class="px-2.5 py-1 rounded-lg bg-brand-500/10 text-brand-400 text-[10px] font-bold uppercase tracking-wider block mb-2">{{ str_replace('_', ' ', $order->status) }}</span>
                                @endif
                ...
                @if(isset($orders) && count($orders) > 0)
                    ...
                    <div class="bg-slate-800/40 border border-slate-700/20 rounded-2xl p-5 space-y-4">
                        <div class="flex justify-between items-start border-b border-slate-800 pb-3">
                            <div>
                                <p class="text-xs text-brand-400 font-bold uppercase tracking-wider">{{ $order->invoice }}</p>
                                <p class="text-xs text-slate-500 mt-0.5">{{ $order->created_at->format('d M Y, H:i') }}</p>
                            </div>
                            @if($order->payment_status === 'unpaid')
                            ...
                            @else
                                <span class="px-2.5 py-1 rounded-lg bg-brand-500/10 text-brand-400 text-[10px] font-bold uppercase tracking-wider">{{ str_replace('_', ' ', $order->status) }}</span>
                            @endif
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center gap-3 text-sm">
                                <div class="w-10 h-10 bg-slate-800 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0">
                                    <img src="{{ $item->produk->thumbnail_url ?? asset('images/default-product.png') }}" class="w-full h-full object-cover">
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-white font-medium truncate text-xs">{{ $item->product->name ?? $item->produk->name ?? 'Produk' }}</p>
                                    <p class="text-[10px] text-slate-500 mt-0.5">Size: {{ $item->size ?? '-' }} | Qty: {{ $item->quantity }}</p>
                                </div>
                                <p class="text-xs font-semibold text-slate-300 flex-shrink-0">Rp {{ number_format($item->price * $item->quantity, 0, ',', '.') }}</p>
                            </div>
                            @endforeach
                        </div>
                        ...
                        <div class="flex justify-between items-center pt-3 border-t border-slate-800 text-xs">
                            <div>
                                ...
                                <p class="text-sm font-bold text-white mt-0.5">Rp {{ number_format($order->total, 0, ',', '.') }}</p>
                            </div>
                            <button onclick="openOrderDetail('{{ $order->invoice }}')" class="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-white font-medium rounded-lg transition-all border border-slate-700/50">Detail Pesanan</button>
                        </div>
                    </div>
                    @endforeach
                @else
            ...
            <div id="tab-acc-content-profil" class="space-y-4 hidden">
                <form id="acc-profile-form" onsubmit="submitProfile(event)" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" required value="{{ auth()->user()->name }}" class="w-full px-4 py-3 bg-slate-800 border border-slate-700/50 rounded-xl text-white text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all">
                    </div>
                    <div>
                        ...
                        <input type="email" disabled value="{{ auth()->user()->email }}" class="w-full px-4 py-3 bg-slate-800/40 border border-slate-800 rounded-xl text-slate-500 text-sm cursor-not-allowed">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wide">No. Telepon / WhatsApp</label>
                        <input type="tel" name="phone" value="{{ auth()->user()->phone ?? '' }}" placeholder="081234567890" class="w-full px-4 py-3 bg-slate-800 border border-slate-700/50 rounded-xl text-white text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all">
                    </div>
            ...
            <div id="form-login" class="p-6">
                <form method="POST" action="{{ route('login.post') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" required placeholder="contoh@email.com" value="{{ old('email') }}" class="w-full px-4 py-3 bg-slate-800 border border-slate-600/50 rounded-xl text-white placeholder-slate-500 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all">
                    </div>
                    <div>
                    ...
                    @if(session('error'))
                    <div class="text-xs text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-lg px-3 py-2">{{ session('error') }}</div>
                    @endif
            ...
            <div id="form-register" class="p-6 hidden">
                <form method="POST" action="{{ route('daftar') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" required placeholder="Nama lengkap" value="{{ old('name') }}" class="w-full px-4 py-3 bg-slate-800 border border-slate-600/50 rounded-xl text-white placeholder-slate-500 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-1.5">No. WhatsApp <span class="text-rose-400">*</span></label>
                        <input type="tel" name="whatsapp" required placeholder="081234567890" value="{{ old('whatsapp') }}" class="w-full px-4 py-3 bg-slate-800 border border-slate-600/50 rounded-xl text-white placeholder-slate-500 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all">
                    </div>
                    <div>
            ...
            <form id="checkout-form" onsubmit="submitOrder(event)" class="p-6 space-y-5">
                ...
                <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700/20">
                    ...
                    <div class="grid grid-cols-2 gap-3 text-sm">
                        <div><p class="text-slate-500 text-xs">Nama</p><p class="text-white font-medium">{{ auth()->user()->name }}</p></div>
                        <div><p class="text-slate-500 text-xs">WhatsApp</p><p class="text-white font-medium">{{ auth()->user()->whatsapp ?? auth()->user()->phone ?? '-' }}</p></div>
                        <div class="col-span-2"><p class="text-slate-500 text-xs">Email</p><p class="text-white font-medium">{{ auth()->user()->email }}</p></div>
                    </div>
                </div>
                @endauth
                <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700/20">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="provinsi" id="provinsi-select" required onchange="handleProvinceChange()" class="w-full px-4 py-3 bg-slate-800 border border-slate-600/50 rounded-xl text-white text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all">
                            ...
                            @foreach($provinsi ?? [] as $k => $v)<option value="{{ $k }}">{{ $v }}</option>@endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div class="col-span-2">
                        ...
                        <select name="kurir" id="kurir-select" required onchange="updateShipping()" class="w-full px-4 py-3 bg-slate-800 border border-slate-600/50 rounded-xl text-white text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all">
                            ...
                            @foreach($kurir ?? [] as $k => $v)<option value="{{ $k }}">{{ $v }}</option>@endforeach
                        </select>
                    </div>
                </div>
                <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700/20">
                ...
                <div>
                    ...
                    @else
                        <div class="grid grid-cols-{{ min(3, $activePaymentCount) }} gap-3">
                            @if($paymentMethods->get('transfer')?->is_active)
                            ...
                            @endif
                            @if($paymentMethods->get('cod')?->is_active)
                                <label class="flex flex-col items-center justify-center text-center gap-2 p-4 bg-slate-800 border border-slate-700/50 rounded-xl cursor-pointer hover:border-brand-500/50 transition-all has-[:checked]:border-brand-500 has-[:checked]:bg-brand-500/5 group">
                            ...
                            @endif
                            @if($paymentMethods->get('ewallet')?->is_active)
                                <label class="flex flex-col items-center justify-center text-center gap-2 p-4 bg-slate-800 border border-slate-700/50 rounded-xl cursor-pointer hover:border-brand-500/50 transition-all has-[:checked]:border-brand-500 has-[:checked]:bg-brand-500/5 group">
    ...
    <div id="success-modal" class="fixed inset-0 bg-black/70 z-[60] hidden flex items-center justify-center p-4">
        <div class="bg-slate-900 border border-slate-700/30 rounded-2xl w-full max-w-sm text-center p-10">
            ...
            <button onclick="window.location.href = '{{ route('shop') }}?open_account=pesanan'" class="block w-full py-3.5 bg-gradient-to-r from-brand-500 to-brand-600 hover:from-brand-400 hover:to-brand-500 text-white font-semibold rounded-xl transition-all mb-3">Lihat Pesanan Saya</button>
            <button onclick="window.location.href = '{{ route('shop') }}'" class="w-full py-3 border border-slate-600 hover:border-slate-500 text-slate-300 hover:text-white font-medium rounded-xl transition-all">Tutup</button>
        </div>
    </div>
...
let cart = [], currentCategory = '{{ $kategori->isNotEmpty() ? $kategori->first()->id : "semua" }}', shippingCost = 0;
const shippingRates = @json($ongkirRates ?? []);
    ...
    async function submitProfile(e) {
        ...
        try {
            const r = await api('{{ route("api.pembeli.profil") }}', 'POST', data);
        ...
        <div id="unpaid-payment-alert" class="fixed bottom-6 left-6 z-40 max-w-sm bg-[#1c1c2d]/95 backdrop-blur-md border border-amber-500/25 p-4 rounded-2xl shadow-2xl flex gap-3.5 transition-all duration-500 hover:border-amber-500/45 group animate-bounce-subtle">
            ...
            <div class="flex-1 min-w-0 space-y-1">
                ...
                <p class="text-[11px] text-slate-400 leading-normal">
                    Pesanan <span class="font-mono font-semibold text-slate-300">#{{ $unpaidOrder->invoice }}</span> belum dibayar sebesar <strong class="text-amber-400 font-mono font-bold">Rp {{ number_format($unpaidOrder->total, 0, ',', '.') }}</strong>.
                </p>
                <div class="flex items-center gap-3 pt-1">
                    <button onclick="openOrderDetail('{{ $unpaidOrder->invoice }}')" class="text-[11px] font-bold text-brand-400 hover:text-brand-300 transition-colors flex items-center gap-0.5">
