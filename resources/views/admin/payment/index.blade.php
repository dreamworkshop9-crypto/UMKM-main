...
@section('title', 'Metode Pembayaran - SALZA')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <form action="{{ route('admin.payment.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="bg-[#1c1c2d] rounded-xl border border-outline-variant/20 overflow-hidden">
            <div class="p-6 border-b border-slate-700/50 flex items-center justify-between">
                ...
                <div class="relative inline-block w-12 mr-2 align-middle select-none transition duration-200 ease-in">
                    <input type="checkbox" name="transfer_active" id="transfer_active" class="toggle-checkbox absolute block w-6 h-6 rounded-full bg-slate-600 border-4 border-slate-800 appearance-none cursor-pointer focus:outline-none transition-all duration-300" {{ $methods->get('transfer')?->is_active ? 'checked' : '' }}/>
                ...
                <div id="bank-accounts-container" class="space-y-3">
                    ...
                    <div class="bank-row flex flex-col md:flex-row gap-3 items-center bg-slate-900/50 p-4 rounded-xl border border-slate-800/60">
                        <div class="w-full md:w-1/4">
                            ...
                            <input type="text" name="bank_name[]" value="{{ $bank['bank'] ?? '' }}" placeholder="Contoh: BCA" class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all">
                        </div>
                        <div class="w-full md:w-1/3">
                            <label class="block text-xs text-slate-500 mb-1">Nomor Rekening</label>
                            <input type="text" name="bank_account[]" value="{{ $bank['account_number'] ?? '' }}" placeholder="Contoh: 123-456-7890" class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all">
                        </div>
                        <div class="w-full md:w-1/3">
                            <label class="block text-xs text-slate-500 mb-1">Nama Pemilik Rekening</label>
                            <input type="text" name="bank_holder[]" value="{{ $bank['account_name'] ?? '' }}" placeholder="Contoh: ShoesMarket" class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all">
                        </div>
                        <div class="pt-5">
        ...
        <div class="bg-[#1c1c2d] rounded-xl border border-outline-variant/20 overflow-hidden">
            <div class="p-6 border-b border-slate-700/50 flex items-center justify-between">
                ...
                <!-- Toggle Switch -->
                <div class="relative inline-block w-12 mr-2 align-middle select-none transition duration-200 ease-in">
                    <input type="checkbox" name="cod_active" id="cod_active" class="toggle-checkbox absolute block w-6 h-6 rounded-full bg-slate-600 border-4 border-slate-800 appearance-none cursor-pointer focus:outline-none transition-all duration-300" {{ $methods->get('cod')?->is_active ? 'checked' : '' }}/>
        ...
        <div class="bg-[#1c1c2d] rounded-xl border border-outline-variant/20 overflow-hidden">
            <div class="p-6 border-b border-slate-700/50 flex items-center justify-between">
                ...
                <!-- Toggle Switch -->
                <div class="relative inline-block w-12 mr-2 align-middle select-none transition duration-200 ease-in">
                    <input type="checkbox" name="ewallet_active" id="ewallet_active" class="toggle-checkbox absolute block w-6 h-6 rounded-full bg-slate-600 border-4 border-slate-800 appearance-none cursor-pointer focus:outline-none transition-all duration-300" {{ $methods->get('ewallet')?->is_active ? 'checked' : '' }}/>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="ewallet_phone" value="{{ $ewalletDetails['phone'] ?? '' }}" placeholder="Contoh: 0812-3456-7890" class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-300 mb-1.5">Nama Pemilik Akun</label>
                            <input type="text" name="ewallet_holder" value="{{ $ewalletDetails['account_name'] ?? '' }}" placeholder="Contoh: ShoesMarket" class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all">
                        </div>
                        <div>
                    ...
                    <div class="flex flex-col items-center justify-center border border-slate-800 bg-slate-900/40 rounded-xl p-4">
                        ...
                        <div class="w-36 h-36 bg-white p-2 rounded-lg flex items-center justify-center shadow-lg">
                            <img src="{{ $ewalletDetails['qris_image'] ?? '/images/qris_mockup.png' }}" alt="QRIS QR Code" class="max-w-full max-h-full object-contain">
                        </div>
                    </div>
                </div>
            </div>
