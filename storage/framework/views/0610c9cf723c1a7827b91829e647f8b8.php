<?php $__env->startSection('title', 'Ulasan'); ?>
...
<div class="bg-[#1c1c2d] rounded-xl border border-outline-variant/20 overflow-hidden">
    ...
    <div class="px-6 py-5 border-b border-outline-variant/10 flex items-center justify-between">
        ...
        <span class="bg-yellow-500 text-[#121220] text-[11px] font-bold px-2 py-0.5 rounded-full min-w-[24px] h-[24px] flex items-center justify-center"><?php echo e($ulasan->total()); ?></span>
    </div>
    ...
    <div class="px-6 py-4 border-b border-outline-variant/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-3">
            ...
            <input id="searchInput" class="bg-[#121220] border border-outline-variant/30 rounded-md px-4 py-1.5 text-white focus:ring-1 focus:ring-indigo-500 outline-none w-[200px] text-[13px] placeholder-slate-600" type="text" placeholder="Cari ulasan / nama..." value="<?php echo e(request('search')); ?>"/>
        </div>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody>
                <?php if($ulasan->count() > 0): ?>
                    <?php $__currentLoopData = $ulasan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr class="border-b border-outline-variant/5 hover:bg-[#1a1a2e] transition-colors">
                        <td class="px-6 py-4 text-[13px] text-slate-400">
                            <?php echo e(($ulasan->currentPage() - 1) * $ulasan->perPage() + $index + 1); ?>

                        </td>
                        <td class="px-6 py-4 text-[13px] text-slate-300 whitespace-nowrap">
                            <?php echo e($item->created_at->format('d M Y')); ?>

                            <span class="block text-[11px] text-slate-500"><?php echo e($item->created_at->format('H:i')); ?></span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-[13px] text-slate-200"><?php echo e($item->user->name ?? '-'); ?></div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-[13px] text-indigo-400 font-mono"><?php echo e($item->pesanan->code ?? '-'); ?></span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-1">
                                <?php for($i = 1; $i <= 5; $i++): ?>
                                    <span class="material-symbols-outlined text-[16px] <?php echo e($i <= ($item->rating ?? 0) ? 'text-yellow-400' : 'text-slate-600'); ?>" style="font-variation-settings: 'FILL' <?php echo e($i <= ($item->rating ?? 0) ? '1' : '0'); ?>;">star</span>
                                <?php endfor; ?>
                                <span class="text-[11px] text-slate-500 ml-1">(<?php echo e($item->rating ?? 0); ?>)</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-[13px] text-slate-300 max-w-[300px] truncate"><?php echo e($item->review ?? '-'); ?></p>
                        </td>
                        <td class="px-6 py-4">
                            <?php
                                $statusCfg = [
                            ...
                            ?>
                            <span class="text-[11px] font-bold px-2.5 py-1 rounded-full border <?php echo e($statusCfg[$item->status] ?? 'bg-slate-500/15 text-slate-400'); ?>">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex gap-1.5">
                                ...
                                <button onclick="toggleStatus(<?php echo e($item->id); ?>, 'nonaktif')" class="p-1.5 rounded-lg bg-red-500/10 text-red-400 hover:text-white hover:bg-red-500/20 transition-all" title="Nonaktifkan">
                                    <span class="material-symbols-outlined text-[16px]">visibility_off</span>
                                </button>
                                <?php else: ?>
                                <button onclick="toggleStatus(<?php echo e($item->id); ?>, 'aktif')" class="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 hover:text-white hover:bg-emerald-500/20 transition-all" title="Aktifkan">
                                    <span class="material-symbols-outlined text-[16px]">visibility</span>
                                </button>
                                <?php endif; ?>
                                <button onclick="hapusUlasan(<?php echo e($item->id); ?>)" class="p-1.5 rounded-lg bg-red-500/10 text-red-400 hover:text-white hover:bg-red-500/20 transition-all" title="Hapus">
    ...
    <div class="px-6 py-4 border-t border-outline-variant/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <p class="text-[13px] text-slate-500">Showing <?php echo e($ulasan->firstItem() ?? 0); ?> to <?php echo e($ulasan->lastItem() ?? 0); ?> of <?php echo e($ulasan->total()); ?> entries</p>
        <div class="flex">
            <a href="<?php echo e($ulasan->appends(request()->query())->previousPageUrl() ?? '#'); ?>" class="px-4 py-2 border border-outline-variant/30 bg-[#212135] text-slate-400 rounded-l-lg text-[13px] hover:bg-[#2a2a40] hover:text-white transition-colors <?php echo e($ulasan->onFirstPage() ? 'opacity-40 pointer-events-none' : ''); ?>">Previous</a>
            <a href="<?php echo e($ulasan->appends(request()->query())->nextPageUrl() ?? '#'); ?>" class="px-4 py-2 border border-l-0 border-outline-variant/30 bg-[#212135] text-slate-400 rounded-r-lg text-[13px] hover:bg-[#2a2a40] hover:text-white transition-colors <?php echo e($ulasan->hasMorePages() ? '' : 'opacity-40 pointer-events-none'); ?>">Next</a>
        </div>
    </div>
...
function toggleStatus(id, status) {
    ...
    fetch('<?php echo e(route("admin.ulasan.update-status", ":id")); ?>'.replace(':id', id), {
        method: 'POST',
        headers: {
...
function hapusUlasan(id) {
    ...
    fetch('<?php echo e(route("admin.ulasan.destroy", ":id")); ?>'.replace(':id', id), {
        method: 'DELETE',
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/irawan/laravel/shoesmarket/UMKM/UMKM-main/resources/views/admin/ulasan.blade.php ENDPATH**/ ?>